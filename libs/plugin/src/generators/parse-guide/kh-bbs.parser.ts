...
const stepIds = Object.keys(stepIdRecord)
  .map((actId) => Object.keys(stepIdRecord[actId]))
  .flat();
...
export default async function (tree: Tree, options: ParseGuideGeneratorSchema) {
  ...
  rl.on('line', (line) => {
    ...
    if (lineHasId && lineHasId[1] && actIds.includes(lineHasId[1])) {
      ...
      lineHasId &&
      lineHasId[1] &&
      referenceIds.includes(lineHasId[1])
    ) {
  ...
  return new Promise<void>((resolve, reject) => {
    rl.on('close', () => {
      ...
      acts.forEach((section, sectionIndex) => {
        let safeSectionName = names(section.title)
          .fileName.replaceAll('.', '')
          .replaceAll('"', '')
          .replaceAll("'", '')
          .replaceAll('“', '')
        ...
        safeSectionName =
          section.id === 1 ? 'reference' : `${sectionIndex}-${safeSectionName}`;
        const sectionIndexFileName = joinPathFragments(
          'apps/game-guides/app/data/markdown/kh-bbs/',
          safeSectionName,
          '/index.md'
        );
...
${subsection.content
  .map((line, index) => {
    if (
      subsection.content[index + 1] &&
      subsection.content[index + 1].trim().startsWith('¯') &&
      line.trim().length === subsection.content[index + 1].trim().length
    ) {
      return `## ${line} ##`;
    } else if (
